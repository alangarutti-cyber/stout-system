...
const ClientesTab = () => {
  ...
  const handleWhatsAppClick = (cliente) => {
    ...
    if (phone.length < 10) {
      toast({
        title: "Telefone inválido",
        description: "O formato do telefone é inválido.",
        variant: "destructive",
      });
      return;
    }

    const url = `https://api.whatsapp.com/send?phone=55${phone}&text=Olá%20${encodeURIComponent(
      cliente.name || ""
    )},%20tudo%20bem?%20Aqui%20é%20da%20Stout%20Group!`;
  ...
  const handleExportTemplate = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Clientes_Modelo");
    XLSX.writeFile(workbook, "modelo_clientes.xlsx");

    toast({
      title: "Modelo exportado com sucesso!",
      description: "O arquivo modelo_clientes.xlsx foi baixado.",
    });
  };
  ...
  const handleExportClientes = () => {
    const exportData = filteredClientes.map((cliente) => ({
      name: cliente.name || "",
      document1: cliente.document1 || "",
      document2: cliente.document2 || "",
      ...
      default_payment_condition: cliente.default_payment_condition || "",
    ...
    XLSX.writeFile(workbook, "clientes_exportacao.xlsx");

    toast({
      title: "Exportação concluída!",
      description: "O arquivo clientes_exportacao.xlsx foi baixado.",
    });
  };
  ...
  const handleImport = async (e) => {
    ...
    try {
      ...
      const normalizedRows = rows.map((row) => {
        const creditLimit =
          row.credit_limit && String(row.credit_limit).trim() !== ""
            ? parseFloat(
        ...
        return {
          ...
          person_type: row.person_type || "Física",
          credit_limit: creditLimit,
          default_payment_condition: row.default_payment_condition || "",
          observations: row.observations || "",
          status: row.status || "Ativo",
          image_url: row.image_url || "",
          company_id: row.company_id || null,
          last_edited_by: authUser?.id ?? null,
      ...
      if (!validRows.length) {
        toast({
          ...
          description: "O arquivo não contém registros com nome preenchido.",
          variant: "destructive",
        });
        return;
      }

      const { error } = await supabase.from("clientes").insert(validRows);
      ...
      toast({
        ...
        description: `${validRows.length} cliente(s) importado(s) com sucesso.`,
      });
    ...
    <div className="space-y-6 p-6">
      <div className="flex flex-wrap gap-4 justify-between items-center">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button variant="outline" onClick={handleExportClientes}>
            <FileDown className="mr-2 h-4 w-4" />
            Exportar
          </Button>
                ...
                <tr className="text-muted-foreground">
                  <th className="p-3 w-10">
                    <Checkbox
                      ...
                      checked={
                        selectedClientes.length === filteredClientes.length &&
              ...
              <tbody>
                {filteredClientes.map((cliente) => (
                  <tr key={cliente.id} className="border-b hover:bg-muted/50">
                    <td className="p-3">
                      <Checkbox
                        onCheckedChange={() => handleSelectionChange(cliente.id)}
                    ...
                    <td className="p-3 text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent>
                          ...
                          <DropdownMenuItem
                            onClick={() => handleCobrancaClick(cliente)}
      ...
      <Dialog open={isCobrancaOpen} onOpenChange={setIsCobrancaOpen}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              Nova Cobrança para {clienteParaCobranca?.name}
            </DialogTitle>
          </DialogHeader>

          <CobrancaForm
            clientePredefinido={clienteParaCobranca}
            onSaveSuccess={() => setIsCobrancaOpen(false)}
...
const ClienteForm = ({
    ...
    if (cliente) {
      setFormData((prev) =>
        Object.keys(initialState).reduce((acc, key) => {
          acc[key] = cliente[key] ?? prev[key] ?? initialState[key];
          return acc;
        }, {})
      );
    } else {
  ...
  const handleImageUpload = async (e) => {
    ...
    const { error } = await supabase.storage
      .from("private_files")
      .upload(`clientes_fotos/${fileName}`, file);
    ...
    if (error) {
      ...
      const {
        ...
        .from("private_files")
        .getPublicUrl(`clientes_fotos/${fileName}`);
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.document1) {
      ...
      if (checkError) {
        toast({
          title: "Erro ao validar CPF/CNPJ",
    ...
    const credit_limit_numeric = formData.credit_limit
      ? parseFloat(
    ...
    if (error) {
      toast({
        title: `Erro ao ${cliente ? "atualizar" : "criar"} cliente`,
        description: error.message,
        variant: "destructive",
      });
    } else {
      toast({
        title: `Cliente ${cliente ? "atualizado" : "criado"} com sucesso!`,
      });
      onSaveSuccess();
    }
  };
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="md:col-span-3 grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <Label>{formData.person_type === "Física" ? "CPF" : "CNPJ"}</Label>
            <Input name="document1" value={formData.document1} onChange={handleChange} />
          </div>

          <div>
            <Label>
              {formData.person_type === "Física" ? "RG" : "Inscrição Estadual"}
            </Label>
            <Input name="document2" value={formData.document2} onChange={handleChange} />
          </div>
      ...
      <div className="space-y-4 border-t pt-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <Input
              name="default_payment_condition"
              value={formData.default_payment_condition}
              onChange={handleChange}
            />
          </div>
  ...
  if (isDialog) return formContent;
...
const CobrancaForm = ({
  ...
  const handleSubmit = async (e) => {
    ...
    const valorNumerico = parseFloat(
      formData.valor.toString().replace(/\./g, "").replace(",", ".")
    );
    ...
    const dataToSave = {
      ...
      data_pagamento:
        formData.status === "Pago" ? new Date().toISOString().split("T")[0] : null,
    };
    ...
    >
      ...
      <div>
        ...
        <Input
          ...
          onChange={handleChange}
          placeholder="https://seu-link.com/pagar"
        />
      </div>
...
const BulkEditDialog = ({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Editar {selectedIds.length} Clientes em Massa</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setUpdateData((p) => ({ ...p, status: e.target.value }))
              }
              className="w-full p-2 border rounded-md bg-background"
            >
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setUpdateData((p) => ({ ...p, company_id: e.target.value }))
              }
              className="w-full p-2 border rounded-md bg-background"
            >
              <option value="">Manter original</option>
